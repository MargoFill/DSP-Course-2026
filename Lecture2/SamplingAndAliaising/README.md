# Lecture 02 – Sampling and Aliasing

 ## Objective

 Briefly describe the purpose of this investigation.

 ## Nyquist Analysis

 $`fN=fmax/2 => fN=10Hz/2 =5Hz`$

1) Which sampling frequencies satisfy the Nyquist criterion?
   Any which are no more than twice smaller than maximum frequency
2) Is sampling exactly at the Nyquist rate recommended in practice? Explain your answer.
   Yes.

 ## Results

 Describe the observations for each sampling frequency.

 ## Aliasing Discussion

 Explain where aliasing occurred and why.

 ## Engineering Recommendation

 Recommend an appropriate sampling frequency and justify your choice.

 ## AI Usage

 - AI Tool Used

- Prompt(s)

- Summary of AI Response

- What I Modified

- How I Verified the Results
